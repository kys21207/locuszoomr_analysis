...
scatter_plot <- function(loc,
  ...
  # Modify shapes based on cs_num and PIP
  if ("cs_num" %in% colnames(data) & "PIP" %in% colnames(data)) {
    pch <- ifelse(is.na(data$PIP), pch, as.numeric(as.factor(data$cs_num)) + 20)
  }
  ...
  # Add PIP values to the plot
  if ("PIP" %in% colnames(data)) {
    par(new = TRUE)
    plot(data[, loc$pos], data$PIP * 10, type = "p", axes = FALSE, xlab = "", ylab = "", pch = 16, col = "red")
    axis(4, at = pretty(range(data$PIP * 10)), labels = pretty(range(data$PIP)), las = 1, cex.axis = cex.axis)
    mtext("Posterior Inclusion Probability (PIP)", side = 4, line = 3, cex = cex.lab)
  }
}
